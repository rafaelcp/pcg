...
class Reg
{
	...
	static public var maxScore:Int = 0;
	...
	static public function saveData() {
		Reg.save.data.level = Reg.level;
		Reg.save.data.maxScore = Reg.maxScore;
		Reg.save.flush();
	}
	
	static public function loadData() {
		if (Reg.save.data.level != null) Reg.level = Reg.save.data.level;
		if (Reg.save.data.maxScore != null) Reg.maxScore = Reg.save.data.maxScore;
	}
}