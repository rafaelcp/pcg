...
class Sfxr
{
	...
	{
		var v: Array <String> = s.split (",");
		for (i in 0...v.length)
			if (v[i] == "")
				v[i] = "0";
		resetParams ();
		
		wave_type = Std.parseInt (v[0]); // waveType
		p_env_attack = Std.parseFloat (v[1]); // attackTime
		p_env_sustain = Std.parseFloat (v[2]); // sustainTime
		p_env_punch = Std.parseFloat (v[3]); // sustainPunch
		p_env_decay = Std.parseFloat (v[4]); // decayTime
		p_base_freq = Std.parseFloat (v[5]); // startFrequency
		p_freq_limit = Std.parseFloat (v[6]); // minFrequency
		p_freq_ramp = Std.parseFloat (v[7]); // slide
		p_freq_dramp = Std.parseFloat (v[8]); // deltaSlide
		vib_amp = Std.parseFloat (v[9]); // vibratoDepth
		vib_speed = Std.parseFloat (v[10]); // vibratoSpeed
		p_arp_mod = Std.parseFloat (v[11]); // changeAmount
		p_arp_speed = Std.parseFloat (v[12]); // changeSpeed
		p_duty = Std.parseFloat (v[13]); // squareDuty
		p_duty_ramp = Std.parseFloat (v[14]); // dutySweep
		p_repeat_speed = Std.parseFloat (v[15]); // repeatSpeed
		p_pha_offset = Std.parseFloat (v[16]); // phaserOffset
		p_pha_ramp = Std.parseFloat (v[17]); // phaserSweep
		p_lpf_freq = Std.parseFloat (v[18]); // lpFilterCutoff
		p_lpf_ramp = Std.parseFloat (v[10]); // lpFilterCutoffSweep
		p_lpf_resonance = Std.parseFloat (v[20]); // lpFilterResonance
		p_hpf_freq = Std.parseFloat (v[21]); // hpFilterCutoff
		p_hpf_ramp = Std.parseFloat (v[22]); // hpFilterCutoffSweep
		master_vol = Std.parseFloat (v[23]); // masterVolume
	}
